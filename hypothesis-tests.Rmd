...
```{r}
lm_coef_stat <- . %>% lm(score~bty_avg, data=.) %>% coef %>% t %>% as_tibble %>% rename(intercept="(Intercept)")
evals_ch5 %>% lm_coef_stat
...
evals_bootstrap %>% summarize(p=ci_probs(0.95), 
...
```{r}
ggplot(evals_bootstrap) + aes(x=intercept, y=bty_avg) + geom_point() + geom_vline(xintercept=c(3.7,4.1), col="red") + geom_hline(yintercept=c(0.033,0.099), col="blue")
```
...
ggplot(evals_ch5) + aes(x=bty_avg, y=score) + geom_point() + 
  geom_abline(aes(intercept=intercept, slope=bty_avg), data=evals_bootstrap %>% slice_sample(n=100), alpha=0.1) 
